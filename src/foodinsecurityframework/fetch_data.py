...
def fetch_csv_data(
  ...
  if not clean_cols_names:
    ...
    df.rename(columns={col: clean_colnames(col) for col in cols}, inplace=True)
...
def fetch_geo_data(csv_path: str) -> geopandas.GeoDataFrame:
  ...
  region_df = geopandas.GeoDataFrame(
      data=geo_df.filter(items=[c for c in geo_df.columns if c != 'geometry']),
      geometry=geopandas.GeoSeries.from_wkt(geo_df['geometry']),
...
def fetch_weights(weights_path: str,
  ...
  with open(weights_path, mode='r', encoding='utf-8') as infile:
    ...
    if not clean_cols_names:
      ...
      weights_dict = {
          clean_colnames(str(rows[0])): int(rows[1]) for rows in reader
      }
  return weights_dict
  ...
  for i in range(begin_offset, len(df) - end_offset, number_lists):
    ...
    if regex_search is not None:
      df.at[i + number_lists - 1,
            'name_scenario'] = 'df_' + regex_search.group().removesuffix('_')
    else:
...
def join_data(df: pd.DataFrame,
  ...
  if clean_cols_names:
    ...
    df_all.rename(columns={col: clean_colnames(col) for col in cols},
                  inplace=True)
